...
import com.itangcent.easyapi.core.cache.http.HttpContextCacheHelper
...
import com.itangcent.easyapi.core.grpc.GrpcStatus
...
class EndpointDetailsPanel(
    ...
    private fun checkGrpcCallReady(): Boolean {
        if (!project.settings<GrpcSettings>().grpcCallEnabled) {
        ...
        val resolver = com.itangcent.easyapi.core.grpc.GrpcRuntimeResolver.getInstance(project)
    ...
    private fun openGrpcSettings() {
        com.itangcent.easyapi.core.settings.ui.EasyApiSettingsConfigurable.selectTab(
            com.itangcent.easyapi.core.settings.ui.EasyApiSettingsConfigurable.TAB_GRPC
        )
        com.intellij.openapi.options.ShowSettingsUtil.getInstance().showSettingsDialog(
            project,
            com.itangcent.easyapi.core.settings.ui.EasyApiSettingsConfigurable::class.java
        )
    }
     ...
     * [EndpointVariableResolver.RESOLVED_BODY_JSON_KEY] so that [com.itangcent.easyapi.channel.curl.CurlFormatter]
     * picks it up instead of re-serializing the original [com.itangcent.easyapi.core.psi.model.ObjectModel] body.
     *
    ...
    fun buildEditedEndpoint(): ApiEndpoint? {
        ...
        if (editedBody != null) {
            // String literal for curl.resolvedBodyJson key (avoids channel.curl import from core.*)
            extensions["curl.resolvedBodyJson"] = editedBody
        }
    ...
    private fun setupEnvContextMenu() {
        val syncService = com.itangcent.easyapi.channel.spi.EnvironmentSyncSupport.getInstance(project)

        envComboBox.componentPopupMenu = JPopupMenu().apply {
            add(JMenuItem("Push to Postman").apply {
                isEnabled = syncService.isAvailable()
                if (!isEnabled) toolTipText = "Configure Postman API token in settings"
                addActionListener {
                    syncService.showSyncDialogAndExecute(
                        com.itangcent.easyapi.channel.spi.EnvironmentSyncSupport.SyncMode.PUSH
                    )
                }
            })
            add(JMenuItem("Pull from Postman").apply {
                ...
                addActionListener {
                    syncService.showSyncDialogAndExecute(
                        com.itangcent.easyapi.channel.spi.EnvironmentSyncSupport.SyncMode.PULL
                    ) {
                        loadEnvironments()
                    }
                }
