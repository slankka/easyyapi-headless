...
object MetaAnnotationResolver {
    ...
    suspend fun hasMetaAnnotation(
        ...
        hasMetaAnnotationImpl(element, targetAnnotations)
    }

    /**
     * Non-suspend variant of [hasMetaAnnotation] for use from non-suspend SPI methods
     * (e.g., [ApiClassRecognizer.matchesClass] which is called from line-marker providers
     * via `runBlocking`). Uses [readSync] to acquire read access synchronously.
     */
    fun hasMetaAnnotationSync(
        element: PsiModifierListOwner,
        targetAnnotations: Set<String>
    ): Boolean = readSync {
        hasMetaAnnotationImpl(element, targetAnnotations)
    ...
    private fun hasMetaAnnotationImpl(
        element: PsiModifierListOwner,
        targetAnnotations: Set<String>
    ): Boolean {
        // Annotation types are never API classes, even if meta-annotated with framework annotations
        if (element is PsiClass && element.isAnnotationType) return false
